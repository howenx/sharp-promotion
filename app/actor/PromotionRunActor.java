 ...
 */
public class PromotionRunActor extends AbstractActor {

    @Inject
    public PromotionRunActor(SkuService itemService, WSClient ws, Configuration configuration) {
            ...
            if (versionVoList != null && versionVoList.size() > 0) {
                ...
                F.Promise<File> filePromise = ws.url(configuration.getString("zip.download.url") + URLEncoder.encode(versionVo.getDownloadLink(), "UTF-8")).get().map(response -> {
                    ...
                    try {
                        ...
                        Logger.error("promotion.zip.path文件为: " + file.getPath());
